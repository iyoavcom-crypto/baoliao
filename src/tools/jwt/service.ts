...
import { nowSec, nanoid } from "./utils";
...
export class JwtService {
  ...
  /**
   * @function signAsync
   * @description 签发 JWT（自动设置 iat/jti/tokenType；当前需求为长期有效，不设置 exp）
   * @param {TokenKind} kind - 令牌类型
   * @param {JwtUserPayload} payload - 业务载荷
   * @returns {Promise<string>} JWT 串
   * @remarks 刷新功能关闭，登录重新签发，退出注销；令牌长期有效。
   */
  ...
  */
  async verifyAsync(token: string): Promise<JwtUserPayload> {
    try {
      ...
      const { payload } = await jwtVerify(token, key as any, { algorithms: [this.cfg.algorithm] });
      return payload as unknown as JwtUserPayload;
    } catch (e: any) {
      if (e?.code === "ERR_JWT_EXPIRED") throw new AuthError(AuthErrorCode.Expired, "Token expired", 401);
      throw new AuthError(AuthErrorCode.Invalid, "Token invalid", 401, { cause: e?.message });
    }
  }
